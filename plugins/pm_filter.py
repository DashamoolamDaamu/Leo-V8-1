...
#ALERT FN IN SPELL CHECK FOR LANGAUGES TO KNOW HOW TO TYPE MOVIES esp english spell check goto adv spell check to check donot change the codes      
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.START_TXT.format(mention, temp.U_NAME, temp.B_NAME),
...
async def auto_filter(client, msg, spoll=False):
    ...
    # NOTE: Smart Filter Wizard is NOT used in normal Auto Filter.
    # Normal searches show results immediately, same as original ShobanaFilterBot V8.
    # Wizard is only used inside Browse Mode (browse_menu, genre_browse, actor_search).
